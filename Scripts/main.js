 ...
 * -------------------------------------------
 */
var BlueSlide = Figure.extend({
	init: function(parent) {
		this._super(parent.x + 0, parent.y + 0, parent.level);
		this.parent = parent;
// 		this.setImage(images.sprites, 191, 366);
		// this.setImage(images.spritesfb, 241, 243);
		this.setSize(16, 16);
		this.direction = parent.direction;
		this.vy = 0;
		this.life = Math.ceil(2000 / constants.interval);
		this.speed = constants.walking_v*2;
		this.vx = this.direction === directions.right ? this.speed : -this.speed;
		envoipossible=0
	},
	setVelocity: function(vx, vy) {
...
var Mario = Hero.extend({
	init: function(x, y, level) {
		...
		else if (this.marioState === mario_states.blueshell)
		this.setImage(images.spritesfb, 81, 0);
		else
	...
	input: function(keys) {
		...
		if (glisse==1){
			...
			if(keys.accelerate && this.marioState === mario_states.blueshell){
				this.slide();
			}
		}
		
		if(!this.crouching && glisse==0) {
		...
		if(keys.accelerate && keys.left || keys.accelerate && keys.right)
			velo+=1
		else
			velo=0
		
		if (velo>10 && this.marioState === mario_states.blueshell){
			glisse=1
		}
	...
	victory: function() {
		...
		this.setImage(images.spritesfb, this.state === size_states.small ? 241 : 161, 81);
		else
	...
	slide: function() {
		if(!this.cooldown && envoipossible==1) {
			this.cooldown = constants.cooldown;
			this.level.playSound('shoot');
			new BlueSlide(this);
		}
	...
	shooter: function() {
		...
		if (this.marioState !== mario_states.blueshell)
			this.setMarioState(mario_states.fire);
	},
	
	sheller: function() {
				this.grow();
				this.setMarioState(mario_states.blueshell);
	},
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			...
			this.setImage(images.spritesfb, 0, 243);
	...
	walkLeft: function() {
		if(this.state === size_states.small) {
			...
			this.setImage(images.spritesfb, 81, 162);
			else
	...
	stand: function() {
		...
		this.setImage(images.spritesfb, coords.x, coords.y);
		else
...
var Luigi = Hero.extend({
	init: function(x, y, level) {
		...
		else if (this.luigiState === luigi_states.blueshell)
		this.setImage(images.spriteslfb, 81, 0);
		else
	...
	input: function(keys) {
		this.fast = keysluigi.accelerate;
		if (glisse==1)
			this.crouching = true;
		else
		this.crouching = keysluigi.down;
		
		if (glisse==1){
			
			if(this.onground && keysluigi.up){
			this.jump();
			}
			
			if(keysluigi.accelerate && this.luigiState === luigi_states.blueshell){
				this.slide();
			}
		}
		...
		if(!this.crouching && glisse==0) {
			...
			if(keysluigi.accelerate && this.luigiState === luigi_states.fire)
				this.shoot();
				
			if(keysluigi.right || keysluigi.left)
				this.walk(keysluigi.left, keysluigi.accelerate);
			else
				this.vx = 0;
		...
		if(keysluigi.accelerate && keysluigi.left || keysluigi.accelerate && keysluigi.right)
			velo+=1
		else
			velo=0
		
		if (velo>10 && this.luigiState === luigi_states.blueshell){
			glisse=1
		}
		
		if(!keysluigi.accelerate){
		glisse=0
		envoipossible=1
		}
	...
	victory: function() {
		document.getElementById("finish").innerHTML ='<img id="finish_level" src="Content\/luigi-finish-2.gif">'
		this.level.playMusic('success');
		this.clearFrames();
		this.view.show();
		if (this.luigiState === luigi_states.normal)
		...
		this.setImage(images.spriteslfb, this.state === size_states.small ? 241 : 161, 81);
		else
	...
	shooter: function() {
		...
		if (this.luigiState !== luigi_states.blueshell)
			this.setLuigiState(luigi_states.fire);
	},
	
	sheller: function() {
				this.grow();
				this.setLuigiState(luigi_states.blueshell);
	},
	
	walk: function(reverse, fast) {
		if (keysluigi.speedfast)
		this.vx = constants.walking_v * (fast ? 15 : 1) * (reverse ? - 1 : 1);
		else		
		this.vx = constants.walking_v * (fast ? 2 : 1) * (reverse ? - 1 : 1);
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			...
			this.setImage(images.spriteslfb, 0, 243);
	...
	walkLeft: function() {
		if(this.state === size_states.small) {
			...
			this.setImage(images.spriteslfb, 81, 162);
			else
	...
	stand: function() {
		...
		this.setImage(images.spriteslfb, coords.x, coords.y);
		else
	...
	move: function() {
		this.input(keys);		
		this._super();
		if (keysluigi.gravity)
		this.jump()
	    if (keysluigi.gravitydeux)
		this.vy = constants.gravity ;
		if (keysluigi.grow){
		if(this.state === size_states.big){
		this.hurt()
		...
		if (keysluigi.shooter)
		this.shooter();
	
		if (keysluigi.sheller)
		this.sheller();
...
var Enemy = Figure.extend({
	...
	hit: function(opponent) {
		...
		if(opponent instanceof Mario) {
			...
			else if(keys.down && opponent.marioState === mario_states.blueshell)
			{
			var s = this.speed * Math.sign(this.speed);
			this.setVelocity(this.direction === directions.right ? -s : s, this.vy)
	...
	hit: function(opponent) {
		...
		if(this.vx) {
			if(this.idle)
				...
				if (opponent.marioState === mario_states.blueshell && keys.down)
				{
				if (this.direction===directions.left){
				this.setPosition(this.x+30, this.y);
...
var BlueTurtleShell = Enemy.extend({
	...
	hit: function(opponent) {
		if(opponent instanceof Mario) {
				opponent.grow();
				opponent.setMarioState(mario_states.blueshell);
			this.die();
		}
		if(opponent instanceof Luigi) {
				opponent.grow();
				opponent.setLuigiState(luigi_states.blueshell);
			this.die();
		}
		if(opponent instanceof Peach) {
				opponent.grow();
				opponent.setPeachState(peach_states.blueshell);
			this.die();
		}
	},
	collides: function(is, ie, js, je, blocking) {		
...
var BlueTurtle = Enemy.extend({
	init: function(x, y, level) {
		...
		this.deathCount = 0;
		this.setSize(34, 54);
		this.setBlueShell(new BlueTurtleShell(x, y, level));
	},
	setBlueShell: function(blueshell) {
		if(this.blueshell || this.wait)
			return false;
			
		this.blueshell = blueshell;
		blueshell.hide();
		this.setState(size_states.big);
		return true;
	},
	...
	setVelocity: function(vx, vy) {
		...
		if(!this.setupFrames(6, 2, rewind, label))
			this.setImage(images.enemiesl, coords.x, coords.y);
	},
	die: function() {
		this._super();
		this.clearFrames();
		...
		if(this.deathMode === death_modes.normal) {
			this.deathFrames = Math.floor(600 / constants.interval);
			this.setImage(images.enemiesl, 102, 437);
		} else if(this.deathMode === death_modes.shell) {
			this.level.playSound('shell');
			this.setImage(images.enemiesl, 68, (this.state === size_states.small ? (this.direction === directions.right ? 437 : 382) : 325));
		}
	},
	death: function() {
		if(this.deathMode === death_modes.normal)
	...
	hurt: function(opponent) {	
		...
		this.hide()
		this.die()
		this.blueshell.activate(this.x, this.y);
		this.blueshell = undefined;
	},
}, 'blueturtle');
